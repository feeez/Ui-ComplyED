      ...
      <div className="flex flex-col gap-4">
        <div className="bg-[#191820] rounded-xl border border-white/[0.08] overflow-hidden">
          ...
          <div className="px-6 py-5 border-b border-white/[0.06] flex items-center justify-between">
            <div>
              <p className="text-[15px] font-semibold text-white/75">Compliance Rules</p>
              <p className="text-[12px] text-white/30 mt-0.5">{activeCount} active · {tabRules.length} total in this tab</p>
            </div>
            <motion.button
              ...
              transition={{ duration: 0.15 }}
              className="text-[12px] font-semibold border border-white/[0.14] rounded-lg px-4 py-2 text-white/65 hover:text-white/85 transition-all cursor-pointer"
            >
              + Tambah Aturan
            </motion.button>
          </div>
          ...
          {/* Tabs + Search */}
          <div className="px-6 py-3.5 border-b border-white/[0.05] flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="flex items-center gap-1 bg-white/[0.03] rounded-lg p-1 flex-1 relative">
              {TABS.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => { setActiveTab(tab.id); setSearch(""); }}
                  className="relative flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-md cursor-pointer z-10"
                >
                  {activeTab === tab.id && (
                    <motion.div
                  ...
                  <span className={`relative text-[13px] font-semibold transition-colors ${activeTab === tab.id ? "text-white/80" : "text-white/35"}`}>
                    {tab.label}
                  </span>
                  <span className={`relative text-[11px] font-medium px-1.5 py-0.5 rounded transition-colors ${
            ...
            <div className="relative sm:w-60">
              ...
              <input
                ...
                className="w-full bg-white/[0.05] border border-white/[0.08] rounded-lg pl-8 pr-3 py-2 text-[13px] text-white/65 placeholder-white/20 outline-none focus:border-white/20 focus:bg-white/[0.08] transition-all"
              />
            </div>
          </div>
          ...
          <div className="divide-y divide-white/[0.05] min-h-[200px]">
            <AnimatePresence mode="popLayout">
              {pageRules.length > 0 ? pageRules.map((rule, i) => (
                ...
                >
                  <span className="text-[11px] font-mono text-white/20 w-20 flex-shrink-0">{rule.id}</span>
                  <p className={`flex-1 text-[14px] font-medium truncate transition-colors ${rule.enabled ? "text-white/75" : "text-white/30"}`}>
                    {rule.name}
                  </p>
                  <Badge variant={rule.enabled ? "active" : "inactive"}>
                    {rule.enabled ? "Active" : "Inactive"}
                  </Badge>
                  <span className="text-[12px] text-white/20 flex-shrink-0 hidden sm:block">{rule.updated}</span>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button className="text-[12px] font-medium text-white/35 hover:text-white/70 transition-colors cursor-pointer border border-white/[0.08] rounded-lg px-3 py-1.5 hover:border-white/18">
                      Edit
                    </button>
                    <button className="text-[12px] font-medium text-red-400/40 hover:text-red-400/80 transition-colors cursor-pointer border border-red-500/[0.08] rounded-lg px-3 py-1.5 hover:border-red-500/20">
                      Hapus
                    </button>
                  </div>
                </motion.div>
