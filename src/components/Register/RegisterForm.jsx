...
export default function RegisterForm() {
  ...
  const validateStep1 = () => {
    const e = {};
    if (!form.name.trim()) e.name = "Nama lengkap diperlukan";
                  ...
                  >
                    <motion.div variants={fieldItem}>
                      <Field
                        ...
                        onBlur={() => {
                          if (!form.name.trim())
                            setErrors((p) => ({ ...p, name: "Nama lengkap diperlukan" }));
                        }}
                        error={errors.name}
                        autoComplete="name"
                      />
                    ...
                    <motion.div variants={fieldItem}>
                      <Field
                        ...
                        value={form.username}
                        onChange={(e) => { set("username")(e); clearErr("username"); }}
                        onBlur={() => {
                          if (!form.username.trim())
                            setErrors((p) => ({ ...p, username: "Username diperlukan" }));
                          else if (form.username.length < 3)
                            setErrors((p) => ({ ...p, username: "Minimal 3 karakter" }));
                          else if (/\s/.test(form.username))
                            setErrors((p) => ({ ...p, username: "Username tidak boleh mengandung spasi" }));
                        }}
                ...
                >
                  ...
                  >
                    <p className="text-white/40 text-[13px] leading-relaxed mb-5">
                      Akun dibuat untuk{" "}
                      <span className="text-white/70 font-medium">{form.username}</span>
                    </p>
